...
# ── 云端 OpenAI 兼容模型 ─────────────────────────────────────
# 默认项目配置使用 gpt-5.4；运行前需设置 OPENAI_API_KEY。
MODEL_CONFIGS["gpt-5.4"] = ModelConfig(
    url=os.environ.get("OPENAI_BASE_URL", "https://openai.drawthings.cn/v1"),
    model_name=os.environ.get("OPENAI_MODEL", "gpt-5.4"),
    api_key=os.environ.get("OPENAI_API_KEY", ""),
    max_len=32768,
    use_vllm_extra_body=False,
)
# 兼容旧配置名。
MODEL_CONFIGS["gpt-5.4-judge"] = MODEL_CONFIGS["gpt-5.4"]
    ...
    **kwargs,
    ...
    for attempt in range(config.retry_attempts):
        try:
            ...
            resp = config.client.chat.completions.create(**request_kwargs)
